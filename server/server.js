...
var GeneSchema = new Schema({
  ...
  { collection: 'geneTable' });

var promoter_hMSCSchema = new Schema({

},
  { collection: 'promoter_hMSC' });

var promoter_OBchema = new Schema({
},
  {collection: 'promoter_OB' });

var promoter_OCSchema = new Schema({
},
  ...
  try {
    ...
    if (cell === "hMSC") {
      ...
      if (SigHic){
        ...
        let promoters = await promoter_hMSC.find({ "$and": [{ "HiC_Distal_bin": reg }, { "HiC_Promoter_bin": { "$in": proms } }] })
      ...
      if (SigHic){
        ...
        let promoters = await promoter_OB.find({ "$and": [{ "HiC_Distal_bin": reg }, { "HiC_Promoter_bin": { "$in": proms } }] })
        res.send({
          "snps": snps
          , "promoters": promoters
        }
      ...
      if (SigHic){
        ...
        let promoters = await promoter_OC.find({ "$and": [{ "HiC_Distal_bin": reg }, { "HiC_Promoter_bin": { "$in": proms } }] })
        res.send({
          "snps": snps
          , "promoters": promoters
        }
...
app.get("/api/disease_test2/:disease", async (req, res, next) => {
  ...
  try {
    const disease = await Disease2SNP.find({ "Disease_trait": req.params.disease })
...
app.get("/api/range_test/:coordinates", async (req, res, next) => {
  ...
  // let myquery = { $and: [{ "Start": { $gte: start } }, { "Start": { $lte: end } }, { "#Chr": chr }] }
  let myquery={ $and: [{"#Chr": chr}, { "Start": { "$gte": start } }, { "Start": { "$lte": end } }] };
  console.log(myquery)
  try{
