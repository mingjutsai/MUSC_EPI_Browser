...
import { gene_columns, snp_columns, disease_columns, essential_column_ids } from "./TableViewConfig"
...
function SortableTableHead(props) {
  const { classes, onSelectAllClick, order, orderBy, numSelected, rowCount, onRequestSort, wf } = props;
  ...
  return (
    <TableHead>
      <TableRow>
        <TableCell />
        {columns.filter(
          column => props.showDetails ? true : essential_column_ids.includes(column.id)
        ).map((column) => (
          <TextOnlyTooltip TransitionComponent={Zoom} placement="top-start" style={{ zIndex: 88 }} title={column.description}>
            <TableCell
              key={column.id}
              align={column.align}
              style={{ minWidth: column.minWidth, fontSize: "1.7em", color: column.color, textDecorationLine: "underline", textDecorationStyle: "dashed", textDecorationColor: "black" }}
...
export default function TableView(props) {
      ...
      if (props.keepSNPinPromoters && "Promoter" === row.snp_bin_anno) {
        ...
        for (let criteria of Object.keys(props.cutoff)) {
          ...
          if (row[criteria] > props.cutoff[criteria]) {
            valid_row = false
            break
          } else if (props.cutoff[criteria] !== 1 && (row[criteria] === "" || row[criteria] === undefined)) {
            valid_row = false
            break
          }
        }
  ...
  } else if (props.wf === "snp" && filteredRows !== undefined && filteredRows.length > 0) {
    console.log(filteredRows)
    info["title"] = filteredRows[0].rsid;
    info["position"] = "Chr" + filteredRows[0].snp_id.split("-")[0] + " : " + filteredRows[0].snp_id.split("-")[1] ;
    info["consequence"] = filteredRows[0].region;
    info["nearest_gene"] = filteredRows[0].gene;
    info["maf"] = filteredRows[0].maf
    info["dbsnp_link"] = "https://www.ncbi.nlm.nih.gov/snp/" + filteredRows[0].rsid
    info["gwas_link"] = "https://www.ebi.ac.uk/gwas/variants/" + filteredRows[0].rsid
  } else if(props.wf ==="gene" && filteredRows !== undefined && filteredRows.length > 0){
  ...
  return (
    <div style={{ textAlign: "left", marginTop: 20, marginLeft: 20 }}>
      ...
      <FormControlLabel
        ...
        label="Show details"
      />
      <Paper className={classes.root} style={{ marginTop: 20, textAlign: "center" }}>
          ...
          <Table stickyHeader aria-label="sticky table" style={{
            ...
            <TableBody >
              {stableSort(filteredRows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  return (
                    <TableRow hover tabIndex={-1} key={row.rsid + row.gene + row.ref + row.alt + row.hic_distal_bin} value={row.viewConfig} selected={selectedID === row.rsid + row.alt + row.hic_distal_bin}
                      classes={{ selected: classes.selected }}
                      className={classes.tableRow}>
                      <TableCell>
                        <TextOnlyTooltip TransitionComponent={Zoom} placement="middle-start" style={{ fontSize: "2em", zIndex: 88 }} title="Click row to view details">
                          <IconButton aria-label="expand row" color="primary" size='medium' onClick={() => { handleRowClick(row.viewConfig, row) }}>
                            <CenterFocusWeakIcon style={{ fontSize: "large" }} />
                          </IconButton>
                        </TextOnlyTooltip>
                      </TableCell>
                      {columns.filter(
                        // toggle to showdetails
                        column => showDetails ? true : essential_column_ids.includes(column.id)
                      ).map((column) => {
                        const value = row[column.id];
                        if (column.id === "eqtl") {
                          return (
                            <TableCell key={column.id} align={column.align} style={{ fontSize: '1.4rem', color: "blue", textDecorationLine: "underline", textDecorationStyle: "dashed", textDecorationColor: "blue", cursor: "pointer" }}
                              onClick={() => { handleEqtlClick(row) }}
                            >
                              eQTL info
                            </TableCell>
                          )
                          ...
                          return (
                            <TableCell key={column.id} align={column.align} style={{ fontSize: '1.4rem', textDecorationLine: "underline", textDecorationStyle: "dashed" }}>
                              {/* <List> */}
                              {
                                ...
                                value.map((tf) => {
                                  ...
                                  return (
                                    ...
                                    <HtmlTooltip
                                      ...
                                      {
                                        <React.Fragment>
                                          <Typography style={{ fontSize: 16 }} color="inherit">{tf.name}</Typography>
                                          <img src={"http://jaspar.genereg.net/static/logos/svg/" + tf.jaspar_id + ".svg"}
                                            width="200"
                                            height="100"></img>

                                          <p style={{ fontSize: 16 }}>
                                            <span style={{ letterSpacing: "2px", fontFamily: "Courier New" }}>REF: {tf.seq}</span><br />
                                            <span style={{
                                            ...
                                            <span style={{
                                              ...
                                              fontFamily: "Courier New",
                                              letterSpacing: "2px"
                                            }}>{tf.seq.slice(0, tf.snp_pos)}</span>
                                            <span style={{ color: "red", letterSpacing: "2px", fontFamily: "Courier New", }}>{tf.snp_alt}</span>
                                          </p>
                                        </React.Fragment>
                                      }
                                    >
                                ...
                                value.map((_tf) => {
                                  ...
                                  return (
                                    ...
                                    <HtmlTooltip
                                      ...
                                      {
                                        <React.Fragment>
                                          <Typography style={{ fontSize: 16 }} color="inherit">{_tf.name}</Typography>
                                          <img src={"http://jaspar.genereg.net/static/logos/svg/" + _tf.jaspar_id + ".svg"}
                                            width="200"
                                            height="100"></img>
                        ...
                        else if (column.id == "promoter" || column.id == "hic_distal_bin") {
                          return (
                            <TableCell key={column.id} align={column.align} style={{ fontSize: '1.4rem' }}>
                              {column.format && typeof value === 'number' ? column.format(value) : value}
                              <div />
                              <Link href={`http://ec2-3-236-122-176.compute-1.amazonaws.com:8080/api/range_test/${value}`} target="_blank">
                          ...
                          return (
                            <TableCell key={column.id} align={column.align} style={{ fontSize: '1.4rem' }}>
                              {column.format && typeof value === 'number' ? column.format(value) : value}
                              <div />
                              <Link href={`http://ec2-3-236-122-176.compute-1.amazonaws.com:3000`} target="_blank">
                                (Download SNP table within this range)
                              </Link>
                            </TableCell>
        ...
        <TextOnlyTooltip TransitionComponent={Zoom} placement="top-start" style={{ fontSize: "2em" }} title="Download table">
          <IconButton aria-label="cancel"
            ...
            style={{ cursor: 'pointer', alignText: "left", width: '40px', zIndex: 88 }}>
