...
export default function Dashboard() {
  ...
  const handleChangeTracks = (trackList) => {
    ...
    for (let track of viewConfig.views.tracks.top) {
      if (
        track.tilesetUid === "NyITQvZsS_mOFNlz5C2LJg" ||
  ...
  const createDataGene = (
    ...
    let viewConfig = _.cloneDeep(
      cellType.startsWith("hMSC")
        ? hMSC_ViewConfig
        : cellType.startsWith("OB")
        ? OB_ViewConfig
        : OC_ViewConfig
    );
    viewConfig.views[0].initialXDomain[0] = chr_pos_map[chr] + start - 100000;
  ...
  const createDataSNP = (
    ...
    viewConfig.views[0].initialXDomain[0] =
      chr_pos_map[snp_chr] + snp_start - 100000;
    viewConfig.views[0].initialXDomain[1] =
      chr_pos_map[snp_chr] + snp_start + 100000;
    ...
    let promoter_tx =
      promoter_tx_name !== ""
        ? promoter_tx_name + "(" + promoter_tx_tpm + ")"
        : "";
    let promoter_gene =
      promoter_gene_name !== ""
        ? promoter_gene_name + "(" + promoter_gene_tpm + ")"
        : "";

    let maf = {
      kg_AFR: kg_AFR !== null ? kg_AFR : 0,
      kg_ALL: kg_ALL !== null ? kg_ALL : 0,
      kg_AMR: kg_AMR !== null ? kg_AMR : 0,
      kg_EAS: kg_EAS !== null ? kg_EAS : 0,
      kg_EUR: kg_EUR !== null ? kg_EUR : 0,
      kg_SAS: kg_SAS !== null ? kg_SAS : 0,
      gnomAD_AFR: gnomAD_AFR,
      gnomAD_AMR: gnomAD_AMR,
      gnomAD_All: gnomAD_All,
  ...
  const createDataDisease = (
    ...
    let snp_region =
      region_ensembel !== "" ? region_ensembel.split(";")[0] : "";
    ...
    if (wf === "snp") {
      ...
      for (let item of tableData) {
        let row = createDataDisease(
          ...
          item["OR-Beta"],
          item.CI95,
          item.GeneName_ID_Ensembl,
          item["STUDY_ACCESSION"],
          item["GeneInfo_DistNG_Ensembl"],
          item["chromHMM_E026_25"],
          item["OpenChromatin_hMSC"],
          item["ATAC-MBPS_hMSC-Day1"],
          item["SigHiC-hMSC"]
        );
  ...
  return (
    <div className={classes.root}>
      ...
      >
        <Toolbar className={classes.toolbar}>
          ...
          <img
            ...
            onClick={turnOnWelcome}
            onMouseLeave={changeHeightSmall}
            onMouseOver={changeHeightLarge}
          />
          ...
          >
            <IconButton color="inherit">
              <Badge color="secondary">
                <a
                  href="https://github.com/yzhang250/Musculoskeletal-epigenome-Browser#musculoskeletal-epigenome-browser"
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ color: "black" }}
                >
          ...
          >
            <IconButton color="inherit">
              <Badge color="secondary">
                <a
                  href="https://github.com/yzhang250/Musculoskeletal-epigenome-Browser"
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ color: "black" }}
                >
          ...
          >
            <IconButton color="inherit">
              <Badge color="secondary">
                <a
                  href="mailto:yzhang250@gmail.com"
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ color: "black" }}
                >
      ...
      >
        ...
        {welcome ? (
          <Welcome handleWf={handleWf} handleDrawerOpen={handleDrawerOpen} />
        ) : (
          // <Box height="75%" style={{ textAlign: "left", position: "relative", display: "block", padding: 1 }}>
          // </Box>}
          <div>
            <TableView
              handleChangeViewConfig={handleChangeViewConfig}
